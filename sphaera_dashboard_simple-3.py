...
EM_MARKETS = {
    ...
    'Taiwan': {'index': 'EWT', 'currency': 'TWD=X', 'yield_10y': 1.5, 'policy_rate': 2.00, 'flag': '🇹🇼'},
    'Japan': {'index': 'EWJ', 'currency': 'JPY=X', 'yield_10y': 1.2, 'policy_rate': 0.25, 'flag': '🇯🇵'},
...
for idx, (country, info) in enumerate(EM_MARKETS.items()):
    ...
    # Determine yield direction arrow
    if yield_change > 0.1:
        yield_direction = '↑'  # Rising yields (RED - bad for bonds)
    elif yield_change < -0.1:
        yield_direction = '↓'  # Falling yields (GREEN - good for bonds)
    else:
...
def color_yield_direction(val):
    """Color direction arrows"""
    if val == '↑':  # Rising yields (bad for bonds)
        return 'color: #ff4444; font-size: 18px; font-weight: bold'
    elif val == '↓':  # Falling yields (good for bonds)
        return 'color: #44ff44; font-size: 18px; font-weight: bold'
    return 'color: #888888; font-size: 18px'

# Display options
display_cols = st.multiselect(
    "Select columns to display:",
    options=['Flag', 'Country', 'Index', 'Price', '1M %', '3M %', 'YTD %', 'FX 1M %', '10Y Yield', 'Yield Δ', 'Direction', 'Policy Rate', 'Term Premium'],
    default=['Flag', 'Country', 'Index', '1M %', 'YTD %', 'FX 1M %', '10Y Yield', 'Yield Δ', 'Direction', 'Policy Rate']
)
...
if display_cols:
    ...
    # Color direction arrows
    if 'Direction' in display_cols:
        styled_df = styled_df.applymap(color_yield_direction, subset=['Direction'])
...
chart_type = st.radio(
    "Select chart type:",
    ["1-Month Performance", "YTD Performance", "FX Performance", "Yield Comparison", "Yield Changes", "Term Premium"],
    horizontal=True
)
