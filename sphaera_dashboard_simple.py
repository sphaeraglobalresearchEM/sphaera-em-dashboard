...
if not valid_data.empty:
    ...
    with col3:
        ...
        st.metric(
            "📊 Average Return",
            f"{avg:.1f}%",
            f"{len(valid_data[valid_data['1M %'] > 0])}/{len(valid_data)} positive"
        )
...
display_cols = st.multiselect(
    ...
    options=['Flag', 'Country', 'Index', 'Price', '1M %', '3M %', 'FX 1M %', '10Y Yield', 'Policy Rate', 'Term Premium'],
    default=['Flag', 'Country', 'Index', '1M %', 'FX 1M %', '10Y Yield', 'Policy Rate']
)
...
if display_cols:
    ...
    perf_cols = [col for col in ['1M %', '3M %', 'FX 1M %'] if col in display_cols]
    if perf_cols:
...
chart_type = st.radio(
    "Select chart type:",
    ["1-Month Performance", "FX Performance", "Yield Comparison", "Term Premium"],
    horizontal=True
)
